<script lang="ts">
  ...
  import { uploadRAGrelevantDocumentsSchüler } from "$lib/api/rag";
  import { getDocumentsSchüler } from "$lib/api/rag/get-documents-schueler";
  ...
  import { deleteRagDocumentsSchuelerByID } from "$lib/api/rag/delete-rag-documents-schueler-by-id";
  ...
  async function uploadFile(file: FileUploadFile) {
    await uploadRAGrelevantDocumentsSchüler(file.file);
  }

  function showFile(fileID: string, filetype: string) {
    showFileURL = `/api/v1/rag/bafoeg/schueler/${fileID}`;
    fileTypeIndicator = filetype;
    dialogShowFileIsOpen = true;
  }

  async function deleteDocument(fileID: string) {
    const result = await deleteRagDocumentsSchuelerByID(fileID);
    if (!result) {
  ...
  async function loadData(hash: string = "") {
    ragDocumentPromise = getDocumentsSchüler(filterSearchTerm);
  }
...
<Dialog bind:isOpen={dialogUseRagIsOpen} dialogContent={useRagDialog} />
...
<div class="actions">
  ...
  />
  {#if $userState?.permissions.includes("upload:rag-management-schueler-files")}
