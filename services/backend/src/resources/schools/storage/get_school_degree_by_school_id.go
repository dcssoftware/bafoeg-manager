...
func (s *SchoolStorage) GetSchoolDegreeBySchoolID(tx *sqlx.Tx, page uint, schoolID string) ([]serviceModel.SchoolDegreeModel, customerrors.ErrorInterface) {
	...
	if tx != nil {
		rows, err = tx.Queryx(sqlquery, offset, limit, schoolID)
	} else {
		rows, err = s.db.Queryx(sqlquery, offset, limit, schoolID)
	}
