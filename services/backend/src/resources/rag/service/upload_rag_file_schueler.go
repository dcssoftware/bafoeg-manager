...
func (s *RAGService) UploadRAGrelevantDocumentsPDFSchüler(tx *sqlx.Tx, filename, filetype string, filesize int64, filecontent []byte, createdFromUserID string) error {
	...
	documentID, documentIDErr := s.storage.InsertDocumentSchüler(tx, filename, filetype, fileSizeUint, fileHash, createdFromUserID)
	if documentIDErr != nil {
		return documentIDErr
	}

	// store file in s3
	s.storageS3.UploadDocumentSchüler(documentID.String(), filecontent)
