...
func (s *RAGService) DeleteRagDocumentSchueler(tx *sqlx.Tx, documentID uuid.UUID) customerrors.ErrorInterface {
	...
	fileRefErr := s.storage.DeleteRagFileSchueler(tx, documentID)
	if fileRefErr != nil {
		return fileRefErr
	}
	...
	vectorErr := s.storage.DeleteRagVectorSchueler(tx, documentID)
	if vectorErr != nil {
		return vectorErr
	}
