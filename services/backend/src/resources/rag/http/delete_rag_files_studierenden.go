...
func (h *RAGHandler) DeleteRagFilesStudierenden(c fiber.Ctx) error {
	...
	deleteErr := h.service.DeleteRagDocumentStudierenden(nil, uuid.MustParse(fileIDString))
	if deleteErr != nil {
		status, message := deleteErr.HTTPError()
