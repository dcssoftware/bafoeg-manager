...
func (h *RAGHandler) UploadRAGrelevantDocumentsSchüler(c fiber.Ctx) error {
	...
	err := h.service.UploadRAGrelevantDocumentsPDFSchüler(nil, pdfFile.Filename, kind.MIME.Value, pdfFile.Size, fileContent, userID)
	if err != nil {
		return c.Status(http.StatusInternalServerError).SendString(customerrorconst.INTERNAL_SERVER_ERROR_MESSAGE)
	}
