...
func (s *RAGStorage) InsertDocumentSchüler(tx *sqlx.Tx, fileName, fileType string, fileSize uint, fileHash string, createdFromUserID string) (uuid.UUID, customerrors.ErrorInterface) {
	var result storageModels.IDModel
	var row *sqlx.Row

	sqlquery := `
		INSERT INTO pgvector_rag_schuelerbafoeg_files(
			file_name,
			file_type,
			file_size,
			file_hash,
	...
	if err != nil {
		...
		default:
			return uuid.Nil, customerrors.NewDatabaseError(err, "", "Cannot insert rag relevant file", sqlquery, sqlErrorData)
		}
	}
