...
func (a *UserStore) GetPermissionsByID(tx *sqlx.Tx, userID string) ([]string, customerrors.ErrorInterface) {
	...
	if err := json.Unmarshal(permissionsJSONB, &permissions); err != nil {
		...
		return []string{}, customerrors.NewDatabaseError(
			err,
			userID,
			"Cannot unmarshal user permissions from jsonb database",
			sqlquery,
			data,
		)
	}
