...
func (s *ApplicationsStorage) GetApplicationsMetricsUserAssigned(tx *sqlx.Tx, userID string) (uint, customerrors.ErrorInterface) {
	sqlquery := `
		SELECT COUNT(*) AS "count" FROM applications
		WHERE status IN (
				SELECT id FROM application_status
				WHERE identifier NOT IN ('DENIED', 'APPROVED')
		)
	...
	if tx != nil {
		err = tx.QueryRowx(sqlquery, userID).StructScan(&model)
	} else {
		err = s.db.QueryRowx(sqlquery, userID).StructScan(&model)
	}
