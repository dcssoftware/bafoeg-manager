...
func (s *ApplicationsStorage) GetApplicationRevisionsByApplicationIDCount(tx *sqlx.Tx, applicationID string) (uint, customerrors.ErrorInterface) {
	sqlquerybuilder := squirrel.Select(`COUNT(id) AS count`).From("application_revisions").PlaceholderFormat(squirrel.Dollar)
	if applicationID != "" {
		sqlquerybuilder = sqlquerybuilder.Where(squirrel.Eq{"application_id": applicationID})
	}
