...
func (s *CronjobRagVectorProcessorStorage) SetProcessedFileStatusStudierenden(tx *sqlx.Tx, id uuid.UUID, errorlog string) customerrors.ErrorInterface {

	var err error
	sqlquery := `UPDATE pgvector_rag_studierendenbafoeg_files SET processed_timestamp = now(), processed_error = $2 WHERE id = $1 RETURNING id`
