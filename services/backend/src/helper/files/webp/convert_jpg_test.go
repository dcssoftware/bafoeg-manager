...
func TestConvertJPGToWebP(t *testing.T) {
	...
	for _, file := range files {
		...
		if !file.IsDir() && (extention == ".jpg" || extention == ".jpeg") {
	...
	for _, tt := range testFiles {
		t.Run("Test Convert "+tt.Name+" from jpg to webp", func(t *testing.T) {
			...
			webpData, err := ConvertJPGToWebP(tt.Content)

			assert.NoError(t, err, "ConvertJPGToWebP should not return an error for: %s", tt.Name)
