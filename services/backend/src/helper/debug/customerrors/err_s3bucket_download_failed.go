...
type S3BucketDownloadFailedError struct {
	ID *uuid.UUID
...
func NewS3BucketDownloadFailedError(err error, errorContextData string) *S3BucketDownloadFailedError {
	file, lineNumber, _ := runtime.GetCallerFunction(1)
	errUuid := uuid.New()

	model := &S3BucketDownloadFailedError{
		ID: &errUuid,

		file:       file,
		lineNumber: lineNumber,

		errorIdentifier: errorconst.ERROR_IDENTIFIER_S3_DOWNLOAD_FAILED,
		httpUserMessage: errorconst.S3BUCKET_DOWNLOAD_FAILED_ERROR_MESSAGE,
...
func (e *S3BucketDownloadFailedError) Error() string {
	return e.error.Error()
}

func (e *S3BucketDownloadFailedError) ErrorType() errorconst.ErrorIdentifier {
	return e.errorIdentifier
}

func (e *S3BucketDownloadFailedError) HTTPError() (int, string) {
