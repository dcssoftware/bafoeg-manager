...
func GenerateRandomHashURL(baseURL string) (string, customerrors.ErrorInterface) {
	...
	if err != nil {
		return "", customerrors.NewInternalServerError(
			err,
			"",
			"Could not generate a simple oauth redirect get parameter hash",
		)
	}
	...
	if err != nil {
		return "", customerrors.NewInternalServerError(
			err,
			"",
			fmt.Sprintf("Couldn't parse URL in oauth callback (func: generateRandomHashURL ; data: %v)", baseURL),
		)
	}
